...
Телеграм-бот доступен по [***ссылке***](https://t.me/russian_qa_bot) (если лимиты бесплатного тарифа еще не исчерпаны). Для хостинга я использовала платформу **Railway**. Однако при развертывании модели на Railway время выполения запроса возросло до почти минуты. Вероятно, Railway предоставляет ограниченные ресурсы для бесплатных проектов, что может влиять на скорость инференса.
